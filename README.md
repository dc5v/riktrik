...
인플럭스DB에 대해 한 번 파헤쳐보며 시계열 데이터베이스에 대해 공부하면서 심플하게 한 번 만들어보았습니다.
...
아래 함수 선언문에 해당하는 값을 연산하여 통계데이터를 사용하기 쉽게 만들었습니다.
...
double c_binary_entropy ( const double *data, size_t length );
/*  변동계수 (Coefficient of Variation, CV) */
double c_cv ( double mean, double standard_deviation );
/*  기대값 (Expected Value) */
double c_expected_value ( const double *data, size_t length );
/*  기하평균 (Geometric Mean) */
double c_geometric_mean ( const double *data, size_t length );
/*  조화평균 (Harmonic Mean) */
double c_harmonic_mean ( const double *data, size_t length );
/*  사분위범위 (Interquartile Range, IQR) */
double c_iqr ( double *data, size_t length );
/*  첨도 (Kurtosis)# */
double c_kurtosis ( const double *data, size_t length, double mean, double standard_deviation );
/*  평균 절대 편차 (Mean Absolute Deviation, MAD) */
double c_mad ( double *data, size_t length, double median );
/*  평균 절대 편차 (Mean Absolute Error, MAE) */
double c_mae ( const double *data, size_t length );
/*  최대 차이 (Maximum Deviation) */
double c_maximum_deviation ( const double *data, size_t length, double mean );
/*  절대 편차 중앙값 (Median Absolute Deviation, MeAD) */
double c_mead ( double *data, size_t length );
/*  평균값 (Mean) */
double c_mean ( const double *data, size_t length );
/*  중위값 (Median) */
double c_median ( double *data, size_t length );
/*  최빈값 (Mode) */
double c_mode ( double *data, size_t length );
/*  평균 제곱 편차 (Mean Squared Error, MSE) */
double c_mse ( const double *data, size_t length, double mean );
/*  왜도(MSkewness) : 중위값 기준 왜도 */
double c_mskewness ( double *data, size_t length, double median );
/*  퍼센트 범위 (PercentRange) */
double c_percent_range ( const double *data, size_t length );
/*  백분위수 (Percentiles) */
double c_percentile ( double *data, size_t length, double percentile );
/*  범위 (Range) */
double c_range ( const double *data, size_t length );
/*  제곱평균 (Root Mean Square, RMS) */
double c_rms ( const double *data, size_t length );
/*  제곱평균 로그 (RMSLE) */
double c_rmsle ( const double *data, size_t length );
/*  표준편차 (Standard Deviation) */
double c_standard_deviation ( const double *data, size_t length, double mean );
/*  절사평균 (Trimmed Mean) */
double c_trimmed_mean ( double *data, size_t length, double trim_ratio );
/*  분산 (Variance) */
double c_variance ( const double *data, size_t length, double mean );
/*  왜도(ZSkewness) : 0 기준 왜도 */
double c_zskewness ( const double *data, size_t length, double mean );
/*  사분위수 (Quartiles) */
void c_quartiles ( double *data, size_t length, double *q1, double *q2, double *q3 );
```
